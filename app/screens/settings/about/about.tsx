...
const About = ({componentId, config, license}: AboutProps) => {
    ...
    useEffect(() => {
        const fetchLoadMetric = async () => {
            ...
            const result = await getLicenseLoadMetric(
                serverUrl,
                config.Version,
                isLicensed,
            );
    ...
    const handleAboutTeam = usePreventDoubleTap(
        useCallback(() => {
            return openURL(Config.WebsiteURL);
        }, [openURL]),
    );

    const handlePlatformNotice = usePreventDoubleTap(
        useCallback(() => {
            return openURL(Config.ServerNoticeURL);
        }, [openURL]),
    );

    const handleMobileNotice = usePreventDoubleTap(
        useCallback(() => {
            return openURL(Config.MobileNoticeURL);
        }, [openURL]),
    );

    const handleTermsOfService = usePreventDoubleTap(
        useCallback(() => {
            return openURL(AboutLinks.TERMS_OF_SERVICE);
        }, [openURL]),
    );

    const handlePrivacyPolicy = usePreventDoubleTap(
        useCallback(() => {
    ...
    const serverVersion = useMemo(() => {
        ...
        return intl.formatMessage(
            {
                id: 'settings.about.server.version.value',
                defaultMessage: '{version} (Build {buildNumber})',
            },
            {version, buildNumber},
        );
    }, [config, intl]);
    ...
    const copyToClipboard = useCallback(() => {
        const appVersion = intl.formatMessage(
            {
                id: 'settings.about.app.version',
        ...
        const server =
            buildNumber === version
                ? intl.formatMessage(
                    {
                        id: 'settings.about.server.version.noBuild',
                ...
                : intl.formatMessage(
                    {
                        id: 'settings.about.server.version',
                        defaultMessage:
                              'Server Version: {version} (Build {buildNumber})',
                    },
                    {version, buildNumber},
                );
        const database = intl.formatMessage(
            {
                id: 'settings.about.database',
                defaultMessage: 'Database: {driverName}',
            },
            {driverName: config.SQLDriverName},
        );
        const databaseSchemaVersion = intl.formatMessage(
            {
                id: 'settings.about.database.schema',
                defaultMessage: 'Database Schema Version: {version}',
            },
        ...
        if (loadMetric !== null) {
            const loadMetricStr = intl.formatMessage(
                {
                    id: 'settings.about.license.load_metric',
                    defaultMessage: 'Load Metric: {load}',
                },
                {load: loadMetric},
            );
            copiedString += `\n${loadMetricStr}`;
        ...
        <SettingContainer testID='about'>
            ...
            <View style={styles.infoContainer}>
                ...
                <View style={styles.group}>
                    ...
                    >
                        {intl.formatMessage({
                            id: 'settings.about.server.version.title',
                ...
                {loadMetric !== null && (
                    <View style={styles.group}>
                        ...
                        >
                            {intl.formatMessage({
                                id: 'settings.about.license.load_metric.title',
                ...
                <View style={styles.group}>
                    ...
                    >
                        {intl.formatMessage({
                            id: 'settings.about.database.title',
                ...
                <View style={styles.group}>
                    ...
                    >
                        {intl.formatMessage({
                            id: 'settings.about.database.schema.title',
