...
import { LoginScreen, ServerScreen } from "@support/ui/screen";
...
describe("Server Login - Connect to Server", () => {
    ...
    it("MM-T4676_1 - should match elements on server screen", async () => {
        ...
        await expect(headerTitleConnectToServer).toHaveText(
            "SICI Vault Platform"
        );
        await expect(headerDescription).toHaveText(
            "Sistema de Comunicacion Interna"
        );
        await expect(serverUrlInput).toBeVisible();
        await expect(serverDisplayNameInput).toBeVisible();
        await expect(displayHelp).toHaveText(
            "Choose a display name for your server"
        );
    ...
    it("MM-T4676_3 - should show invalid url error on invalid server url", async () => {
        ...
        await waitFor(serverUrlInputError)
            .toExist()
            .withTimeout(timeouts.FOUR_SEC);
    ...
    it("MM-T4676_5 - should show login screen on successful connection to server", async () => {
        ...
        if (isIos() && !process.env.CI) {
            ...
            await waitFor(Alert.okayButton)
                .toExist()
